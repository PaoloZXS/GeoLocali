...
app.use((req, res, next) => {
  ...
  console.log('['+process.pid+'] incoming', req.method, req.path, 'codes', codes,
              'ctype', req.headers['content-type'], 'body', req.body);
  try {
    fs.appendFileSync(path.join(__dirname, 'req-log.txt'), `${new Date().toISOString()} pid=${process.pid} ${req.method} ${req.path} codes=${codes} ctype=${req.headers['content-type']} body=${JSON.stringify(req.body)}\n`);
  } catch(e) { console.error('req log write failed', e); }
  next();
});
...
try {
  ...
  console.warn('sharp module not available; images will not be resized. run `npm install sharp` to enable.');
}
...
async function ensureMaxImageSize(buffer) {
  ...
  if (meta.width && meta.height) {
    ...
    console.log('ensureMaxImageSize: resizing from', meta.width, 'x', meta.height, 'to', width, 'x', height,
...
db.execute("ALTER TABLE tblocali_photos ADD COLUMN dropbox_path TEXT;")
  ...
  .catch(e => {
    ...
    if (!/duplicate column name/i.test(e.message)) console.warn('add column error', e.message);
  });
...
addCols.forEach(sql=>{
  db.execute(sql).catch(err=>{
    if(!/duplicate column/i.test(err.message)) console.error('Error adding column to tblocali', err);
  });
});
...
async function refreshDropboxToken() {
  ...
  if (!key || !secret || !refresh) {
    console.log('Dropbox refresh flow not configured; falling back to static DROPBOX_TOKEN');
    return;
  }
  ...
  console.log('refreshed Dropbox access token, expires in', d.expires_in, 'seconds');
}
...
async function uploadToDropbox(buffer, filename) {
  ...
  try {
    ...
    console.warn('create_shared_link failed, falling back to temporary link:', linkErr.message);
    const r2 = await fetch('https://api.dropboxapi.com/2/files/get_temporary_link', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!r2.ok) {
      ...
      throw new Error('temporary link failed: ' + r2.status + ' ' + txt + ' (original error: ' + linkErr.message + ')');
    }
...
function initGooglePhotos() {
  try {
    ...
    if (fs.existsSync(GP_TOKEN_PATH)) {
      oAuth2Client.setCredentials(JSON.parse(fs.readFileSync(GP_TOKEN_PATH, 'utf8')));
      googlePhotosClient = google.photoslibrary({version: 'v1', auth: oAuth2Client});
      console.log('Google Photos client initialized');
    } else {
      console.warn('Google Photos token file missing; please run upload_photos.js to authorize');
    }
  } catch (e) {
...
async function uploadToGooglePhotos(buffer, filename) {
  if (!googlePhotosClient) throw new Error('Google Photos client not configured');
  // upload raw bytes
...
app.all('/save-location', (req, res, next) => {
  console.log('received', req.method, 'on /save-location');
  next();
});
...
app.post('/save-location', async (req, res) => {
  ...
  let { id, latitude, longitude, address, name, type, civic, city, closingDay } = req.body;
  ...
  try {
    if (id !== undefined && id !== null && id !== '') {
      ...
      await db.execute(
        ...
        [latitude, longitude, address || null, name||null, type||null, civic||null, city||null, closingDay||null, id]
      );
    ...
    if (existing.rows.length) {
      ...
      res.setHeader('Content-Type', 'application/json');
      return res.send(JSON.stringify({ success: true, id: existingId, duplicate: true }));
    }

    const result = await db.execute(
      'INSERT INTO tblocali (latitude, longitude, address, name, type, civic, city, closingDay) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [latitude, longitude, address || null, name||null, type||null, civic||null, city||null, closingDay||null]
    );
...
app.post('/upload-photo', uploadMiddleware.single('photo'), async (req, res) => {
  try {
    const localeId = req.body.locale_id;
    if (!localeId || !req.file) {
      return res.status(400).json({ error: 'locale_id and photo file required' });
    }
    // enforce max 6 photos per locale
    try {
      const cntRes = await db.execute('SELECT COUNT(*) AS cnt FROM tblocali_photos WHERE locale_id = ?', [localeId]);
      const cnt = cntRes.rows && cntRes.rows[0] && cntRes.rows[0].cnt;
      if (cnt >= 6) {
        return res.status(400).json({ error: 'Limite foto raggiunto', details: 'Non più di 6 immagini per locale' });
      }
    } catch (cErr) {
    ...
    try {
      ...
      console.warn('image resize failed, continuing with original buffer:', resizeErr.message);
    }
    let url;
    try {
      ...
      return res.status(500).json({ error: 'Dropbox upload failed', details: dbErr.message });
    }
    const dropboxPath = '/geoPhoto/' + filename;
    const result = await db.execute('INSERT INTO tblocali_photos (locale_id, url, dropbox_path) VALUES (?, ?, ?)', [localeId, url, dropboxPath]);
    // lastInsertRowid holds new row id
    const insertId = result.lastInsertRowid ? String(result.lastInsertRowid) : null;
...
app.post('/upload-photo', async (req, res) => {
  ...
  try {
    ...
    if (!localeId || !filename || !data) {
      return res.status(400).json({ error: 'locale_id, filename and data required' });
    }
...
app.get(['/photos','/api/photos'], async (req, res) => {
  ...
  try {
    const result = await db.execute('SELECT id, url, dropbox_path FROM tblocali_photos WHERE locale_id = ?', [localeId]);
      ...
      if (r.dropbox_path) {
        try {
          ...
          if (r2.ok) {
            ...
            finalUrl = info.link;
            db.execute('UPDATE tblocali_photos SET url = ? WHERE id = ?', [info.link, r.id]).catch(()=>{});
          }
        } catch (e) {
...
app.get('/migrate-photos', async (req, res) => {
  try {
    const result = await db.execute('SELECT id, url, locale_id FROM tblocali_photos WHERE dropbox_path IS NULL');
    const rows = result.rows;
    const updated = [];
    if (rows.length === 0) {
    ...
    for (const r of rows) {
      ...
      try {
        const r2 = await fetch('https://api.dropboxapi.com/2/sharing/get_shared_link_metadata', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        if (r2.ok) {
          ...
          if (meta.path_lower) {
            await db.execute('UPDATE tblocali_photos SET dropbox_path = ? WHERE id = ?', [meta.path_lower, r.id]);
    ...
    if (!usedMeta) {
      ...
      let entries = (listJson.entries || []).map(e => ({ path: e.path_lower, time: e.server_modified }));
      ...
      for (const localeId of Object.keys(byLocale)) {
        ...
        for (let i = 0; i < ids.length && idx < entries.length; i++, idx++) {
          ...
          await db.execute('UPDATE tblocali_photos SET dropbox_path = ? WHERE id = ?', [path, rowId]);
          ...
          if (r2.ok) {
            const info = await r2.json();
            await db.execute('UPDATE tblocali_photos SET url = ? WHERE id = ?', [info.link, rowId]);
...
app.post(['/delete-photo','/api/delete-photo'], async (req, res) => {
  ...
  try {
    ...
    const r = await db.execute('SELECT dropbox_path FROM tblocali_photos WHERE id = ?', [photo_id]);
    if (r.rows.length) {
...
app.post('/delete-location', async (req, res) => {
  ...
  try {
    ...
    const photosRes = await db.execute('SELECT id, dropbox_path FROM tblocali_photos WHERE locale_id = ?', [id]);
    ...
    await Promise.all(photos.map(async p => {
      if (token && p.dropbox_path) {
        try {
          ...
          console.warn('unable to delete dropbox file', p.dropbox_path, e.message);
        }
      }
    }));
...
app.use((err, req, res, next) => {
  ...
  if (err.type === 'entity.too.large' || err instanceof SyntaxError && err.message.includes('body')) {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const result = await db.execute('SELECT uid, password_hash, name, approved FROM user WHERE username = ?', [username]);
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const existing = await db.execute('SELECT uid FROM user WHERE username = ?', [username]);
    if (existing.rows.length) {
    ...
    await db.execute('INSERT INTO user (uid, username, password_hash, name, approved) VALUES (?, ?, ?, ?, ?)',
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  try {
    const r = await db.execute('SELECT uid FROM user WHERE username = ?', [username.toLowerCase()]);
    ...
    await db.execute('UPDATE user SET password_hash = ? WHERE uid = ?', [hash, uid]);
...
app.get('/api/session', async (req, res) => {
  ...
  try {
    const result = await db.execute('SELECT name, username, approved FROM user WHERE uid = ?', [payload.uid]);
    if (!result.rows.length) {
      return res.json({ loggedIn: false });
    }
    ...
    res.json({ loggedIn: true, uid: payload.uid, name: user.name, admin: isAdmin });
  } catch (err) {
...
async function ensureAdmin(req, res) {
  ...
  try {
    const r = await db.execute('SELECT username FROM user WHERE uid = ?', [payload.uid]);
    if (r.rows.length && r.rows[0].username === 'paolo.giorsetti@codarini.com') {
      return true;
    }
  } catch (e) {
...
app.post(['/revoke-user','/api/revoke-user'], async (req, res) => {
  console.log('/revoke-user handler invoked', req.method, req.path, 'body', req.body);
  if (!await ensureAdmin(req, res)) return;
  const { uid } = req.body;
  if (!uid) return res.status(400).json({ error: 'uid required' });
  try {
...
app.post('/api/sync', async (req, res) => {
  ...
  console.log('[SYNC] Processing', operations.length, 'offline operations from user', payload.uid);
      ...
      if (url.includes('/locations') && method === 'POST') {
        ...
        try {
          ...
          const result = await db.execute(
            ...
            [locData.name, locData.type, locData.lat, locData.lon, locData.address, locData.civic, locData.city, locData.closing_day, locData.uid, locData.approved]
          );

          operationSuccess = true;
          operationResult = { message: 'Location created', changes: result.rows_affected };
        } catch (e) {
        ...
        try {
          ...
          const photoRes = await db.execute('SELECT dropbox_path FROM tblocali_photos WHERE id = ?', [photo_id]);
          ...
          if (photo?.dropbox_path) {
            ...
            if (token) {
              ...
              }).catch(e => console.warn('[SYNC] Dropbox delete failed:', e.message));
            }
          }
...
app.get(['/pending-users','/api/pending-users'], async (req, res) => {
  if (!await ensureAdmin(req, res)) return;
  try {
    const result = await db.execute('SELECT uid, name, username FROM user WHERE approved = 0');
    res.json(result.rows);
  } catch (e) {
...
app.get('/approved-users', async (req, res) => {
  ...
  if (!await ensureAdmin(req, res)) return;
  try {
    const result = await db.execute('SELECT uid, name, username FROM user WHERE approved = 1');
  ...
  if (app && app.router) {
    ...
    if (app.router.stack) {
      ...
      app.router.stack.forEach((mw, idx) => {
        ...
        if (mw.route) {
          console.log('  route path', mw.route.path, 'methods', mw.route.methods);
        }
        if (mw.name) {
...
app.listen(PORT, () => {
  ...
  if (app && app.router && app.router.stack) {
    ...
    const pendingLayer = app.router.stack.find(l => l.route && l.route.path === '/pending-users');
    if (pendingLayer) {
      ...
      if (pendingLayer.regexp) {
        ...
        console.log('test pending matches /pending-users', pendingLayer.regexp.test('/pending-users'));
        console.log('test pending matches /pending-users/','/pending-users/'.match(pendingLayer.regexp));
      }
